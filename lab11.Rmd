...
Answer: There is now an obvious linear realtionship between the male bird mass and the mass of the eggs.
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  annotate("text", x = 110, y = 3, color = "red", label = "Parrots")
```
...
Answer: Yes, the linear line is what I imagined for the relationship between the egg and male masses.
...
The tropicbird has a male mass of 218.7g and an egg mass of 87.00g. Annotate this point on the graph and give a label for
it:
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  annotate("text", x = 110, y = 3, color = "red", label = "Parrots") +
  geom_smooth(method = "lm", color = "black", se = FALSE, linetype = "dashed", size = 0.5) + 
  annotate("point", x = 218.7, y = 87.00, color = "blue") +
  annotate("text", x = 218.7, y = 110, color = "blue", label = "Tropicbird")
```
...
```{r}
penguins <- filter(birds, type == "Penguin")
```
...
ggplot(birds, aes(male_mass, egg_mass)) +
  ...
  scale_y_log10() +
  geom_point(color = "red", data = parrots) +
  annotate("rect", xmin = 35, xmax = 200, 
                  ymin = 3, ymax = 14,
                  fill = "red", alpha = 0.2) +
  annotate("text", x = 90, y = 2.5, color = "red", label = "Parrots") +
  geom_point(color = "blue", data = penguins) +
  annotate("rect", xmin = 4000, xmax = 15000, 
                  ymin = 90, ymax = 400,
                  fill = "blue", alpha = 0.2) +
  annotate("text", x = 8000, y = 75, color = "blue", label = "Penguins") +
  ggtitle("Egg Mass & Male Mass Correlation: Parrots vs. Penguins")
