...
**Answer**: Yes burrito restaurants seem to have a higher Yelp score and the confidence interval does not include 3.7 so it is not just noise in the data.
...
**Answer**: When collecting the data, they probably collecting information from well known/high scoring burrito restaurants (not all of the burrito restaurants in the area).
...
**Answer**: The slope coefficient gives the additional amount needed to get the mean of the yelp score when chips are included with the burrito.

Is the slope coefficient statistically significant (compared to zero)?

**Answer**: (3.912 + 0.057 = 3.969) No, not statistically significant because contains 0.

Describe this in terms of your answer above.

**Answer**: There is no significant difference between the mean of the yelp score and the mean of the yelp score when chips are included with the burrito.
...
**Answer**: No, confidence interval contains 0.
...
**Answer**: Yes, confidence interval does not contain 0
...
model4 <- lm_basic(overall ~ 1 + synergy_new, data = burrito)
